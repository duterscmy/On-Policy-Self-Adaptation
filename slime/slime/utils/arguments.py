    ...
    def add_slime_arguments(parser):
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Pass enable_thinking=False when applying the chat template.",
            )
        ...
        def add_eval_arguments(parser):
            ...
            parser.add_argument("--eval-rm-type", type=str, default=None)
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                default="grpo",
                help=(
                    "Advantage estimator to use. Select opsa for lowest-logp token training, or add --use-opd with --opd-type for standard on-policy distillation."
                ),
            )
            parser.add_argument(
        ...
        def add_on_policy_distillation_arguments(parser):
            """Add the standard OPD interface and the compact OPSA estimator."""
            parser.add_argument(
                "--use-opd",
                action="store_true",
                default=False,
                help="Enable standard on-policy distillation. --opd-type is required.",
            )
            parser.add_argument(
                "--opd-type",
                type=str,
                ...
                default=None,
                help="Use an external SGLang teacher or a same-architecture Megatron teacher.",
            )
            parser.add_argument(
                "--opd-kl-coef",
                type=float,
                default=1.0,
                help="Coefficient for the sampled-token OPD reverse-KL advantage.",
            )
            parser.add_argument(
                "--opd-teacher-load",
                type=str,
                default=None,
                help="Megatron teacher checkpoint. Required for --opd-type=megatron.",
            )
            parser.add_argument(
                "--opd-teacher-ckpt-step",
                type=int,
                default=None,
                help="Optional Megatron teacher checkpoint iteration.",
            )
            parser.add_argument(
                "--opsa-mode",
                type=str,
...
def slime_validate_args(args):
    ...
    if args.use_slime_router:
        logger.warning(
            "--use-slime-router is deprecated and ignored. slime now always uses sglang_router built from https://github.com/zhuzilin/sgl-router."
        )
    ...
    # Validate the compact standard on-policy distillation interface.
    if args.use_opd:
        if args.opd_type is None:
            raise ValueError("--opd-type must be specified when --use-opd is enabled.")
        if args.opd_kl_coef <= 0:
            raise ValueError("--opd-kl-coef must be > 0 when --use-opd is enabled.")
        if args.opd_type == "megatron":
            if args.opd_teacher_load is None:
                raise ValueError("--opd-teacher-load is required when --opd-type=megatron.")
            ...
            if args.opd_teacher_load is not None:
                raise ValueError("--opd-teacher-load is not valid when --opd-type=sglang.")
            if args.opd_teacher_ckpt_step is not None:
                raise ValueError("--opd-teacher-ckpt-step is only valid when --opd-type=megatron.")
    elif args.opd_type is not None or args.opd_teacher_load is not None or args.opd_teacher_ckpt_step is not None:
        raise ValueError("OPD teacher options require --use-opd.")
    ...
    if args.critic_train_only:
        ...
        if args.actor_num_nodes != 0 or args.actor_num_gpus_per_node != 0:
            raise ValueError(
                f"--critic-train-only requires --actor-num-nodes 0 --actor-num-gpus-per-node 0, but got actor_num_nodes={args.actor_num_nodes}, actor_num_gpus_per_node={args.actor_num_gpus_per_node}."
            )
